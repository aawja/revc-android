...
import com.wn_klaymen1n.revc.LauncherActivity;
...
*/
    ...
    protected static View mNativeLoadingOverlay;
    protected static TextView mNativeLoadingTitle;
    protected static TextView mNativeLoadingSubtitle;
    protected static LinearProgressIndicator mNativeLoadingProgress;
    ...
    public static void initialize() {
        ...
        mLastCursorID = 0;
        mSDLThread = null;
        mNativeLoadingOverlay = null;
        mNativeLoadingTitle = null;
        mNativeLoadingSubtitle = null;
        mNativeLoadingProgress = null;
    ...
    protected void createNativeLoadingOverlay() {
        RelativeLayout overlay = new RelativeLayout(this);
        overlay.setVisibility(View.GONE);
        overlay.setClickable(false);
        overlay.setBackgroundColor(Color.parseColor("#F20F141A"));

        LinearLayout panel = new LinearLayout(this);
        panel.setOrientation(LinearLayout.VERTICAL);
        panel.setPadding(48, 40, 48, 40);
        panel.setBackgroundColor(Color.parseColor("#22000000"));
        ...
        panelParams.setMargins(56, 0, 56, 0);
        ...
        progress.setIndicatorColor(Color.parseColor("#6FE3A1"));
        progress.setTrackColor(Color.parseColor("#294038"));
        ...
        mNativeLoadingOverlay = overlay;
        mNativeLoadingTitle = title;
        mNativeLoadingSubtitle = subtitle;
        mNativeLoadingProgress = progress;
    }

    public static void setNativeLoadingOverlay(final String title, final String subtitle, final boolean visible) {
        if (mSingleton == null) {
            Log.d("SDL", "setNativeLoadingOverlay: mSingleton is null");
            return;
        }
        ...
        mSingleton.runOnUiThread(new Runnable() {
            @Override
            public void run() {
                Log.d("SDL", "setNativeLoadingOverlay: title='" + title + "', subtitle='" + subtitle + "', visible=" + visible);

                if (mNativeLoadingOverlay == null) {
                    Log.d("SDL", "setNativeLoadingOverlay: mNativeLoadingOverlay is null");
                    return;
                }
                ...
                if (shouldRemove) {
                    Log.d("SDL", "setNativeLoadingOverlay: Removing loading overlay (triggered by '" +
                            (title != null && (title.contains("Load scene") || title.contains("Loadscene")) ? title : subtitle) + "')");

                    ViewGroup parent = (ViewGroup) mNativeLoadingOverlay.getParent();
                    if (parent != null) {
                        parent.removeView(mNativeLoadingOverlay);
                        Log.d("SDL", "setNativeLoadingOverlay: Overlay removed from parent");
                    } else {
                        Log.d("SDL", "setNativeLoadingOverlay: Parent is null");
                    }
                    ...
                    Log.d("SDL", "setNativeLoadingOverlay: Overlay references nullified");
                    return;
                }

                Log.d("SDL", "setNativeLoadingOverlay: Setting visibility to " + (visible ? "VISIBLE" : "GONE"));
                mNativeLoadingOverlay.setVisibility(visible ? View.VISIBLE : View.GONE);

                if (title != null && mNativeLoadingTitle != null) {
                    Log.d("SDL", "setNativeLoadingOverlay: Setting title to '" + title + "'");
                    mNativeLoadingTitle.setText(title);
                }
                if (subtitle != null && mNativeLoadingSubtitle != null) {
                    Log.d("SDL", "setNativeLoadingOverlay: Setting subtitle to '" + subtitle + "'");
                    mNativeLoadingSubtitle.setText(subtitle);
                }
            }
        });
    }
...
*/
    ...
    // Settings overlay removed (was unstable on some devices)

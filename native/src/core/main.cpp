...
DoRWStuffEndOfFrame(void)
{
	...
	if (CPad::GetPad(1)->GetLeftShockJustDown() || CPad::GetPad(0)->GetFJustDown(11)) {
		sprintf(s, "screen_%011ld.png", time(nil));
		RwGrabScreen(Scene.camera, s);
	}
#endif
#endif // !MASTER
...
DisplayGameDebugText()
{
	...
	static float SmoothedFrameTimeMs = 1000.0f / 60.0f;
	static float SnappedDisplayFps = 60.0f;
	static const float commonRefreshRates[] = { 30.0f, 45.0f, 60.0f, 72.0f, 90.0f, 120.0f };
	uint32 currentTimeInMillisecondsPauseMode = CTimer::GetTimeInMillisecondsPauseMode();
	if (PreviousTimeInMillisecondsPauseMode != 0 && currentTimeInMillisecondsPauseMode > PreviousTimeInMillisecondsPauseMode) {
		float frameTimeMs = float(currentTimeInMillisecondsPauseMode - PreviousTimeInMillisecondsPauseMode);
		frameTimeMs = Clamp(frameTimeMs, 1.0f, 100.0f);
		float delta = Abs(frameTimeMs - SmoothedFrameTimeMs);
		float blend = delta > 8.0f ? 0.035f : 0.10f;
		SmoothedFrameTimeMs = SmoothedFrameTimeMs * (1.0f - blend) + frameTimeMs * blend;
		float displayFps = 1000.0f / SmoothedFrameTimeMs;
		float snappedFps = displayFps;
		for (float target : commonRefreshRates) {
			float tolerance = Max(1.5f, target * 0.035f);
			if (Abs(displayFps - target) <= tolerance) {
				snappedFps = target;
				break;
			}
		}
		if (Abs(snappedFps - SnappedDisplayFps) > 6.0f)
			SnappedDisplayFps = snappedFps;
		else
			SnappedDisplayFps = SnappedDisplayFps * 0.85f + snappedFps * 0.15f;
		FramesPerSecond = SnappedDisplayFps;
	}
	PreviousTimeInMillisecondsPauseMode = currentTimeInMillisecondsPauseMode;
#else
	FrameSamples++;
	FramesPerSecondCounter += 1000.0f / CTimer::GetTimeStepNonClippedInMilliseconds();
	FramesPerSecond = FramesPerSecondCounter / FrameSamples;

	if ( FramesPerSecondCounter >= 0.5f || FrameSamples > 120 ) {
...
FrontendIdle(void)
{
	...
	CameraSize(Scene.camera, nil, SCREEN_VIEWWINDOW, SCREEN_ASPECT_RATIO);
	CVisibilityPlugins::SetRenderWareCamera(Scene.camera);
	RwCameraClear(Scene.camera, &gColourTop, rwCAMERACLEARZ | rwCAMERACLEARSTENCIL | rwCAMERACLEARIMAGE); //1wn_klaymen1n: Not clearing image here cases bugs on adreno gpu
	if(!RsCameraBeginUpdate(Scene.camera))
		return;

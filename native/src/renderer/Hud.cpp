...
#ifdef ANDROID_SHOW_BUILD_OVERLAY
extern float FramesPerSecond;

static void
DrawAndroidBuildOverlay(void)
{
	char overlay[128];
	wchar overlayW[128];
	...
	snprintf(overlay, ARRAY_SIZE(overlay), "%.0f FPS", FramesPerSecond);
	...
	CFont::SetColor(CRGBA(255, 255, 255, 235));
	CFont::SetDropColor(CRGBA(0, 0, 0, 255));
	CFont::SetDropShadowPosition(1);
	CFont::PrintString(SCREEN_WIDTH * 0.5f, SCREEN_SCALE_Y(16.0f), overlayW);
}
#endif
...
#ifdef ANDROID
				rect.Translate(SCREEN_SCALE_X_FIX(RADAR_LEFT), SCREEN_SCALE_Y(ANDROID_RADAR_TOP_OFFSET));
#else
